(function () {
  ...
  const products = categoryConfigs.flatMap((category, categoryIndex) =>
    category.names.map((name, index) => {
      ...
      return {
        ...
        variants: variantPalette.map((variant, variantIndex) => ({
          id: variant.id,
          name: variant.name,
  ...
  const categories = [{ id: "todos", label: "Todos" }].concat(
  ...
  function summarizeCart(items) {
    const detailedItems = (items || [])
      .map((item) => {
        ...
        return {
          ...
          quantity,
          subtotal,
          subtotalLabel: new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(subtotal),
    ...
    return {
      items: detailedItems,
      total,
      totalLabel: new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(total)
    };
  }

  window.STOREFRONT_DATA = {
    products,
    categories,
    summarizeCart
  };
